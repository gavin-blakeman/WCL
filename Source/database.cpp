...
namespace VWL
{
  ...
  // Function for opening an ODBC database.
  // Reads information from the settings and then creates the database connection.
  ...
  void CDatabase::ODBC()
  {
    ...
    if (!database_.open())
      ERROR(0x0004);		// Unable to contact ODBC driver.
  }

  /// Procedure to open SQLite database.
  /// EXCEPTIONS: 0x000A - DATABASE: Unable to contact SQLite driver
  //
  // 2013-01-25/GGB - Function created.
  ...
  void CDatabase::SQLite()
  {
    ...
    if (driverName.isNull())
      ERROR(0x0002);	// Database settings not correct.
    else
    {
      szConnectionName = QString("ARID");
      ...
      database_.setDatabaseName(settings::settings.value(settings::WEATHER_SQLITE_DATABASENAME,
                                                           QVariant(QString("Data/ARID.sqlite"))).toString());
  ...
  // Function for opening an OrLibAstroClasse XE database. Reads the relevant informaiton
  // from the settings and then creates the database connection.
  // An exception is thrown if the connection cannot be created.
  //
  // 2015-04-01/GGB - Function created.
